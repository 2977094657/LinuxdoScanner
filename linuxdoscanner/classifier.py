...
class TopicClassifier:
    ...
    def _build_user_content(self, *, payload: dict[str, Any]) -> str:
        return (
            ...
            "你必须遵守以下规则：\n"
            "1. 正文证据优先于标题；标题与正文冲突时，以正文为准。对福利或公益站类主题，如果标题或摘要已经明确写出可领取福利、折扣、额度、入口、平台、CDK/邀请码等具体价值点，且正文不矛盾，也可将其视为有效证据，不必强求完整领取步骤。\n"
            ...
            "9. 是否推送只看 requires_notification 这一个字段，系统不会再根据 primary_label 或 labels 二次修正。你必须对这个字段独立负责；拿不准时一律 false。以下情况才明显倾向 true：AI相关+前沿快讯；AI相关+实验复现；AI相关+辟谣实测；真正给出明确做法和经验结论的 Codex技巧/ClaudeCode技巧；以及明确福利或公益站信息。对 Linux.do 的福利类帖子，只要标题、摘要或正文清楚指出可领取福利、折扣、额度、入口、平台、CDK/邀请码、模型范围、开放规则等具体价值点，且正文没有否定，即使没有完整领取步骤也可判 true；若正文明确否定、只是求证真假、纯转述、纯讨论或只是在问怎么领，则判 false。普通求助、订阅纠结、注册庆祝、闲聊、水贴、纯吐槽都应为 false。\n"
